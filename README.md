...
## 13. Thiết Kế Các Bảng Dữ Liệu (Database Design)
...
### 13.1 Bảng `SAN_PHAM` — key `productsLocal` (seed: `database/products-shoes.js`)
...
### 13.2 Bảng `LOAI_SAN_PHAM` — key `categories` / `loaiLocal`
...
### 13.3 Bảng `NHAN_VIEN` — key `employeeList` (seed: `database/employees.js`)
...
### 13.4 Bảng `KHACH_HANG` — key `userList` / `DanhSachNguoiDung`
...
### 13.5 Bảng `DON_HANG` — key `DanhSachDatHang` (user) + `ordersLocal` (admin)
...
### 13.6 Bảng `CHI_TIET_DON_HANG` (lồng trong `order.product` / `order.items`)
...
### 13.8 Bảng `PHIEU_NHAP_HANG` — key `phieuNhapLocal` / `phieuNhapHangLocal`
...
### 13.9 Bảng `CHI_TIET_PHIEU_NHAP` (lồng trong `phieu.chiTiet`)
